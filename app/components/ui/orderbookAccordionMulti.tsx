...
function getHighestSizePriceValue(arr: OrderBookItem[] | undefined): number {
  ...
  return arr.reduce((max, arr) => {
    ...
    return product > max ? product : max; // Return the higher value
  }, 0);
}
...
>(
  (
    { className, children, orderBook, activeView, setActiveView, ...props },
    ref
  ) => {
    const yesAskBook = orderBook?.[0]?.asks.sort(
      (a, b) => Number(b.price) - Number(a.price)
    );
    const yesAskBookHighest = getHighestSizePriceValue(yesAskBook);

    const yesBidBook = orderBook?.[0]?.bids.sort(
      (a, b) => Number(b.price) - Number(a.price)
    );
    const yesBidBookHighest = getHighestSizePriceValue(yesBidBook);

    const noAskBook = orderBook?.[1]?.asks.sort(
      (a, b) => Number(b.price) - Number(a.price)
    );
    const noAskBookHighest = getHighestSizePriceValue(noAskBook);

    const noBidBook = orderBook?.[1]?.bids.sort(
      (a, b) => Number(b.price) - Number(a.price)
    );
    const noBidBookHighest = getHighestSizePriceValue(noBidBook);
                    ...
                    {selectedOrderBook[0]?.map((row, index) => {
                      ...
                      >
                        <td className="p-0 pr-0 mr-0 w-[60%]">
                          <FillAsk
                            value={
                              ((parseFloat(row.price) * parseFloat(row.size)) / selectedOrderBook[2]) *
                              100
                            }
                            className="w-full"
                          />
                        ...
                        <td className="p-2 pl-1 ml-0 w-[40%]">
                          {toTwoDecimal(row.price) + "¢"}
                        </td>
                        <td className="p-2">{toTwoDecimal(row.size)}</td>
                        <td className="p-2">
                          {"$" + toTwoDecimal(parseFloat(row.price) * parseFloat(row.size))}
                        </td>
                      </tr>
                      );
                    })}
                    ...
                    {selectedOrderBook[1]?.map((row, index) => {
                      ...
                      >
                        <td className="hover:bg-[#0a0a0a] p-0 pr-0 mr-0 w-[60%]">
                          <FillBid
                            value={
                              ((parseFloat(row.price) * parseFloat(row.size)) / selectedOrderBook[3]) *
                              100
                            }
